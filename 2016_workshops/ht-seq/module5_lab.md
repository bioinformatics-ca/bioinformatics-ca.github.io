...
Remember the IGV color codes can be found here: [IGV color code] (https://www.broadinstitute.org/igv/interpreting_insert_size)
...
Next, you should download the report generated by snpEff.
...
4. GATK produces even better variant calling results if all three BAM files are specified at the same time (i.e. specifying multiple `-I filename` options). Try this and then perform the rest of module 3 on the trio vcf file. ***Does this seem to improve your variant calling results? Does it seem to reduce the trio conflict rate?***

IGV color codes are available [here] (https://www.broadinstitute.org/igv/interpreting_insert_size).
