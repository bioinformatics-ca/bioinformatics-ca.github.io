...
Optional
????With ViewData buttin you can dowload the data and "look at it"????
NGS: SAMtools/BAM-to-SAM convert BAM to SAM
on Bam sorted by coordinate
 include header in the SAM output (-h)
...
REMOVE_DUPLICATES; default=False (default)

Select validation stringency -> Silent

  METRICS_FILE=alignment/NA12878/NA12878.sorted.dup.metrics No need to give a name
...
reference genome
Select all or Quality score covariate only? 
un-select Tile covariate
